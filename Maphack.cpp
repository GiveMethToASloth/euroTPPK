...
bool RevealLevel(Level* pLevel)
{
  ...
  for (Room2* pRoom2 = pLevel->pRoom2First; pRoom2; pRoom2 = pRoom2->pRoom2Next) {
    ...
    if (!pRoom2->pRoom1) {
      D2COMMON_AddRoomData(pLevel->pMisc->pAct, pLevel->dwLevelNo, pRoom2->dwPosX, pRoom2->dwPosY, pRoom2->pRoom1);
      nAdded = true;
    }
    ...
    if (nAdded)
      D2COMMON_RemoveRoomData(pLevel->pMisc->pAct, pLevel->dwLevelNo, pRoom2->dwPosX, pRoom2->dwPosY, pRoom2->pRoom1);
  }
...
void DrawPresets(Room2* pRoom2)
{
  ...
  {
    int mCell = -1;
    if (pUnit->dwType == UNIT_TYPE_NPC)//Special NPCs.
    {
    ...
    else if (pUnit->dwType == UNIT_TYPE_OBJECT) { //Objects on Map
