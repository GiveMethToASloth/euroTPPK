...
DWORD __fastcall RecvPacket(byte* data, DWORD len) {
	...
	if (data[0] == 0x5b && data[1] == 0x24 && (*(DWORD*)&data[3] == D2CLIENT_GetPlayerUnit()->dwUnitId)) {
		if (GameReady())
		{
			...
			SaveGameName();
			CurrentExp = D2COMMON_GetUnitStat(D2CLIENT_GetPlayerUnit(), 13, 0);
			TimeOn++;
			TimeOn1++;
			if (v_MaxBPs)
				MaxBPs();
	...
	//			if (!CheckPlayerFriend(GetNameByID(PlayerID))) { // om det inte är en polare*/
