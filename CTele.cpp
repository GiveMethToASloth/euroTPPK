...
BOOL CTele::StartTeleport() {
  ...
  for (int i = 0; i < CTele::PathSize; i++) {
    ...
    while (Current < PathAmount) {
      ...
      if (GetDistanceSquared(GetPlayerPosition(D2CLIENT_GetPlayerUnit()->dwUnitId).x, GetPlayerPosition(D2CLIENT_GetPlayerUnit()->dwUnitId).y, Path[Current].x, Path[Current].y) <= 5) {
        Current++;
        Time = 0;
        continue;
      }
